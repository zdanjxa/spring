...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  public void registerFilters() {
    ...
    if (this.markerInterface != null) {
      ...
      acceptAllInterfaces = false; //标记为不接收所有接口
    }
    ...
    // 添加 EXCLUDE 过滤器，排除 package-info.java
  ...
  public Set<BeanDefinitionHolder> doScan(String... basePackages) {
    //执行扫描，获得指定包下符合的类，并封装成 BeanDefinitionHolder 对象的集合
  ...
  private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
    ...
    for (BeanDefinitionHolder holder : beanDefinitions) {
      ...
      //此处实际上是代理对象,将MapperInterface封装成MapperFactoryBean对象, 实际上是 new MapperFactoryBean(MapperInterface)的操作;
      definition.getConstructorArgumentValues().addGenericArgumentValue(definition.getBeanClassName()); // issue #59
      definition.setBeanClass(this.mapperFactoryBean.getClass());

      definition.getPropertyValues().add("addToConfig", this.addToConfig);

      boolean explicitFactoryUsed = false;//是否参照(即是否通过bean name去构建对象)
      if (StringUtils.hasText(this.sqlSessionFactoryBeanName)) {//如果指定了sqlSessionFactory的bean name,则构建ref对象RuntimeBeanReference
        definition.getPropertyValues().add("sqlSessionFactory", new RuntimeBeanReference(this.sqlSessionFactoryBeanName));
        explicitFactoryUsed = true;
      } else if (this.sqlSessionFactory != null) {//否则不为空时赋值sqlSessionFactory
        definition.getPropertyValues().add("sqlSessionFactory", this.sqlSessionFactory);
        explicitFactoryUsed = true;
      }

      //同理设置sqlSessionTemplate
